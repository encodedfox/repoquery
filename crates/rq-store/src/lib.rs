...
pub use traits::{GraphStore, RepoFilter, RepoStore, SortField, SortOrder};
pub use yaml::YamlStore;

use anyhow::{bail, Context, Result};
use rq_core::config::StorageMode;
...
mod tests {
    ...
    fn test_cross_store_equivalence() {
        ...
        for (y, s) in from_yaml
            .repositories
            .iter()
            .zip(from_sqlite.repositories.iter())
        {
            assert_eq!(y.id, s.id);
            assert_eq!(y.metadata.stars, s.metadata.stars);
            assert_eq!(y.metadata.primary_language, s.metadata.primary_language);
            assert_eq!(
                y.quality_metrics.archive_status,
                s.quality_metrics.archive_status
            );
        }
    }
}
