...
pub fn compute_quality_score(repo: &Repository) -> u8 {
    ...
    star_score
        .saturating_add(if repo.metadata.license.is_some() { 5 } else { 0 })
        .saturating_add(if !repo.metadata.description.is_empty() { 3 } else { 0 })
        .min(100)
}

/// Classify activity status for benchmarks.
pub fn classify_activity(repo: &Repository, _active_months: u64, _stale_months: u64) -> &'static str {
...
pub fn create_repo_batch(count: usize) -> Vec<Repository> {
    let langs = ["Rust", "Go", "JavaScript", "Python", "TypeScript", "C++", "Java", "Ruby", "C", "Shell"];
    (0..count)
        .map(|i| {
