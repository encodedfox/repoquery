...
impl GitHubAdapter {
    ...
    pub async fn new(config: &RepoqueryConfig) -> Result<Self> {
        use rq_core::config::CredentialManager;
        ...
        let _user = client
            ...
            .user()
            .await
            .context("Failed to authenticate with GitHub. Check your token permissions.")?;
            ...
            Err(octocrab::Error::GitHub { source, .. }) if source.message.contains("Not Found") => {
                ...
                Ok(Repository {
                    ...
                    metadata: RepositoryMetadata {
                        ...
                        description: "Repository not found - may have been deleted or made private"
                            .to_string(),
                    ...
                    curator_notes: Some(
                        "Repository not found - may have been deleted or made private".to_string(),
                    ),
                    relations: vec![],
                    fork_parent: None,
    ...
    pub async fn check_rate_limit(&self) -> Result<()> {
        ...
        if remaining < self.rate_limit_buffer as usize {
            ...
            let reset = chrono::DateTime::from_timestamp(reset_time as i64, 0).unwrap_or(now);
    ...
    fn convert_repo_to_model(&self, repo: octocrab::models::Repository) -> Result<Repository> {
        ...
        Ok(Repository {
            ...
            quality_metrics: QualityMetrics {
                ...
                last_commit_date: repo.pushed_at.map(|dt| dt.format("%Y-%m-%d").to_string()),
                last_star_update: chrono::Utc::now().format("%Y-%m-%d").to_string(),
