...
impl GitHubGraphQL {
    ...
    pub async fn fetch_starred_page(
        ...
        let repos = conn["edges"]
            ...
            .map(|edges| {
                edges
                    .iter()
                    .filter_map(|e| self.convert_node(&e["node"], Relation::Starred).ok())
                    .collect()
            })
            .unwrap_or_default();
        Ok((repos, end_cursor))
    ...
    fn convert_node(&self, node: &Value, relation: Relation) -> Result<Repository> {
        ...
        let fork_parent = node
            .get("parent")
            .and_then(|p| p["nameWithOwner"].as_str().map(str::to_owned));
        let fork_parent_url = node
            .get("parent")
            .and_then(|p| p["url"].as_str().map(str::to_owned));
...
mod tests {
    ...
    fn test_convert_graphql_node() {
        ...
        assert_eq!(
            repo.metadata.homepage.as_deref(),
            Some("https://www.rust-lang.org")
        );
        assert_eq!(
            repo.quality_metrics.last_commit_date.as_deref(),
            Some("2024-12-01")
        );
...
"#;
        let repo: Repository =
            serde_yml::from_str(yaml).expect("backward compat deserialization failed");
        assert!(repo.fork_parent.is_none());
        assert!(repo.fork_parent_url.is_none());
    }
}
