...
use crate::{redact_sensitive, ProgressTracker, SyncCache, SyncResult};
use anyhow::Result;
use rq_core::config::RepoqueryConfig;
use rq_core::{
    AnomalyType, CanonicalData, DataAnomaly, DataCompletenessMetrics, Relation, Repository,
    SyncQualityReport,
};
...
impl SyncOrchestrator {
    ...
    pub fn new(config: RepoqueryConfig) -> Result<Self> {
    ...
    pub async fn sync_all(&mut self, canonical_path: &Path) -> Result<SyncResult> {
        ...
        for (i, repo) in data.repositories.iter().enumerate() {
            ...
            let is_github_primary = repo
                .platforms
                .iter()
                .any(|p| p.is_primary && matches!(p.platform, rq_core::Platform::GitHub));
        ...
        while let Some(task_result) = join_set.join_next().await {
            match task_result {
                Ok((idx, ident, Ok(updated_repo))) => {
                    ...
                    self.cache
                        .insert(&data.repositories[idx].id, &data.repositories[idx].metadata);
                    synced += 1;
        ...
        info!(
            "Sync complete: {} synced, {} cached, {} failed in {:?}",
            synced, cached, failed, duration
        );
    ...
    pub async fn sync_by_relation(
        ...
        for relation in relations {
            match relation {
                ...
                other => {
                    debug!(
                        "Relation {:?} does not have a fetch strategy, skipping",
                        other
                    );
                }
            }
    ...
    fn merge_fetched_repos(&mut self, data: &mut CanonicalData, fetched: Vec<Repository>) -> usize {
        let mut count = 0;
        for repo in fetched {
            let existing = data.repositories.iter_mut().find(|r| {
                r.metadata
                    .full_name
                    .eq_ignore_ascii_case(&repo.metadata.full_name)
            });
            match existing {
                Some(existing_repo) => {
                    ...
                    self.cache
                        .insert(&existing_repo.id, &existing_repo.metadata);
                }
                None => {
    ...
    pub fn generate_quality_report(
        ...
        for repo in &post_sync_data.repositories {
            if let Some(pre_repo) = pre_map.get(&repo.id) {
                ...
                if !pre_repo.quality_metrics.archive_status && repo.quality_metrics.archive_status {
                    anomalies.push(DataAnomaly {
                        repo_id: repo.id.clone(),
    ...
    pub async fn sync_specific(
        ...
        let identifiers_lower: Vec<String> =
            repo_identifiers.iter().map(|s| s.to_lowercase()).collect();
        ...
        for repo in &mut data.repositories {
            ...
            // Fetch from GitHub
            match github_adapter
                .fetch_repository(&format!("{}/{}", owner, name))
                .await
            {
                Ok(updated_repo) => {
...
mod tests {
    ...
    fn test_quality_report_detects_star_drop() {
        ...
        assert!(
            report
                .anomalies
                .iter()
                .any(|a| matches!(a.anomaly_type, AnomalyType::StarDrop)),
    ...
    fn test_quality_report_detects_newly_archived() {
        ...
        assert!(
            report
                .anomalies
                .iter()
                .any(|a| matches!(a.anomaly_type, AnomalyType::NewlyArchived)),
    ...
    fn test_quality_report_completeness_metrics() {
        ...
        let report = orchestrator
            .generate_quality_report(&pre, &post, 2)
            .unwrap();
