...
enum Commands {
    ...
    /// Analyze repository activity (active/stale/abandoned/trending)
    Activity {
        #[command(subcommand)]
        action: commands::activity::ActivityAction,
    },

    /// Query repositories with filters and output formatting
    Query {
        #[command(subcommand)]
    ...
    Completions {
        /// Shell to generate completions for (omit for all)
        shell: Option<String>,
    },
...
async fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::Query { action } => match action {
            commands::query::QueryAction::List(args) => commands::query::list::run(args).await,
        ...
        Commands::Completions { shell } => {
            ...
            match shell {
                Some(s) => commands::completions::run_completions_shell(&mut cmd, &s)?,
                None => commands::completions::run_completions(&mut cmd)?,
            }
            Ok(())
        }
        ...
        Commands::Man => {
            let mut cmd = Cli::command();
            commands::completions::run_manpage(&mut cmd)?;
            Ok(())
        }
